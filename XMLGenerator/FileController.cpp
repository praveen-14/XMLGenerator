...
void FileController::saveChanges(QList<Change*> *changesHistory, QString path)
{
    ...
    foreach (Change *change, *changesHistory) {
        ...
        textStream << QString("||").toUtf8();
//        QMap<QString,QString> *updates = change->getUpdateData();
//        if(updates->size() > 0){
//            textStream << QString("||").toUtf8();
//            QMap<QString,QString>::iterator updatesIterator = updates->begin();
//            while(updatesIterator != updates->end()){
//                textStream << updatesIterator.key().toUtf8();
//                textStream << QString("->").toUtf8();
//                textStream << updatesIterator.value().toUtf8();
//                updatesIterator++;
//                if(updatesIterator != updates->end()){
//                    textStream << QString("|").toUtf8();
//                }
//            }
//        }
...
QString FileController::saveXML(XMLFileData xmlData, XMLFileData productXMLData, bool newProject)
{
    ...
    if(saveFolder.size() > 0){
        ...
        if(newProject){
            ...
            QString existingProjectFile = QString(xmlFolderPath + "/" + prevFileName + ".meta");
            if(QFile(existingProjectFile).exists()){
                QMessageBox::StandardButton confirmation;
                confirmation = QMessageBox::question(MainWindow::mainWindow, "Warning!", "Project named '" + prevFileName + "' found in the specified directory! Replace existing project?" , QMessageBox::Yes|QMessageBox::No);
...
//        QList<QString> pathArray = folderPath.split("/");
//        if(pathArray.at(pathArray.size()-1).compare(prevFileName) != 0){
//            xmlFolderPath = folderPath + "/" + prevFileName;
//            if(QDir(xmlFolderPath).exists()){
//                QMessageBox::StandardButton confirmation;
//                confirmation = QMessageBox::question(MainWindow::mainWindow, "Attention!", "Existing project named '" + prevFileName + "' found! Save file to that project?" , QMessageBox::Yes|QMessageBox::No);
//                if (confirmation == QMessageBox::No) {
//                    return "aborted";
//                }
//            }else{
//                QDir().mkdir(xmlFolderPath);
//                QMessageBox msg(QMessageBox::Warning, "Attention!", "New Project directory was created as '" + prevFileName + "' in the specified location!");
//                msg.exec();
//            }
//        }else{
//            xmlFolderPath = folderPath;
//            QMessageBox msg(QMessageBox::Warning, "Attention!", "File saved to the selected project '" + prevFileName + "'");
//            msg.exec();
//        }
...
void FileController::saveEnum(Enum *enumObject, QString path){
    ...
    //        qDebug() << enumValuesIterator.key();
    //        qDebug() << enumValuesIterator.value();
...
QString FileController::commit(QString path, QString commitID, bool newProduct, int commitNumber, QString *commitMessage)
{
    ...
    commitFile.open(QIODevice::ReadWrite);

    QString newCommitID;
//    QDate date = QDate::currentDate();
//    QTime time = QTime::currentTime();
//    newCommitID = date.toString("dd.MM.yyyy") + "_" + time.toString("hh-mm-ss.zzz");
...
//        qDebug() << lastVersionDetails.at(0);
//        qDebug() << lastVersionDetails.at(1);
