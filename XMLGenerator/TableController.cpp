...
TableController::TableController(QWidget *tableWidgetParent, CacheConfig *config)
{
    ...
    this->versionTree->setProperty("tableController",qVariantFromValue((void*)this));
...
//XMLFileData* TableController::getOldProductFile(){
//    return oldProductFile;
//}
...
//void TableController::setOldProductFile(XMLFileData *oldProductFile){
//    this->oldProductFile = oldProductFile;
//}
        ...
        for(int columnMapIndex = 0; columnMapIndex < columnMapping.size(); columnMapIndex++){
            ...
            if(productValue.compare(solutionValue) != 0 && dataTable.getAvailableAttributes()->contains(solutionColumnHeaders.at(solutionColumnIndex))){
                ...
                for(int changeItem = 0; changeItem < changes.size(); changeItem++){
                    ...
                    {
//                        qDebug() << change->getFieldName();
//                        qDebug() << change->getAttributeName();
//                        qDebug() << change->getOldValue();
                        found = true;
                        break;
                    }
                }
    ...
    for(int productRow = 0; productRow < newProductRowHeaders.size(); productRow++){
        if(newProductRowHeaders.at(productRow).compare("COMMON") != 0){
            bool found = false;
//            int row = productFile.allFields.indexOf(newProductRowHeaders.at(productRow));
...
//void TableController::cellMoved(int modelIndex, int oldViewIndex, int newViewIndex)
//{
//    bool found = false;
//    QString fieldName = ((QLineEdit*)(tableView->cellWidget(modelIndex,0)))->text();
//    for(int changeItem = 0; changeItem < changes.size(); changeItem++){
//        Change *change = changes.at(changeItem);
//        if(change->getType() == ChangeType::ChangeFieldOrder && change->getFieldName().compare(fieldName) == 0){
//            found = true;
...
//    }
//    if(!found){
//        Change *change = new Change(this, ChangeType::ChangeFieldOrder, fieldName);
//        changes.push_back(change);
//    }
//}
...
//    Change *change = new Change(this, ChangeType::DeleteField, ((QLineEdit*)(tableView->cellWidget(row,0)))->text());
//    changes.push_back(change);
...
//    tableWidget->setSelectionBehavior(QAbstractItemView::SelectColumns);
    ...
    tableView->resizeColumnsToContents();
//    QObject::connect(this->tableView->verticalHeader(),SIGNAL(sectionMoved(int,int,int)),this,SLOT(cellMoved(int,int,int)));
}
...
//    firstCommit->setText(0,version->getVersionID());
...
//        comboBox->setStyleSheet("QComboBox { border-radius: 5px; }");
